...
async function loadSiteHeader() {
  ...
  if (topBar && f.topBarLinks?.length) {
    topBar.innerHTML = f.topBarLinks.map(link =>
      `<a href="${link.href}" class="top-bar-link">${link.label}</a>`
    ).join('');
  }

  // Logo
  const logoEl = document.querySelector('.logo text, .logo');
  if (logoEl && f.logo) {
    const logoText = document.querySelector('.logo text');
    if (logoText) logoText.textContent = f.logo;
  }

  // Nav links
  const navMenu = document.querySelector('.nav-menu');
  if (navMenu && f.navLinks?.length) {
    navMenu.innerHTML = f.navLinks.map(link =>
...
async function loadSiteFooter() {
  ...
  if (footerGrid && f.columns?.length) {
    footerGrid.innerHTML = f.columns.map(col => `
  ...
  if (footerSocial && f.socialLinks?.length) {
    footerSocial.innerHTML = f.socialLinks.map(s => `
  ...
  if (copyright && f.copyrightText) copyright.textContent = f.copyrightText;

  const poweredBy = document.querySelector('.footer-bottom .powered-by');
  if (poweredBy && f.poweredBy) poweredBy.textContent = f.poweredBy;
}
